    ...
    return (
        <div className="max-w-4xl mx-auto py-32 px-4 animate-fade-in-scale">
            ...
            <div className="space-y-6">
                {posts.length === 0 ? (
                    <div className="text-center py-20 text-gray-500 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border-dashed border-2 border-gray-200 dark:border-gray-700">
                        <MessageSquare size={48} className="mx-auto mb-4 opacity-20" />
                        <p>{L.forum.no_posts}</p>
                    </div>
                ) : (
                    posts.map(post => {
                        const canNavigate = !!(post.uid && navigate);
                        const handleProfileClick = () => {
                            if (canNavigate) navigate(`user/${post.uid}`);
                        };

                        return (
                            <div
                                key={post.id}
                                className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 animate-fade-in-up hover:shadow-md transition-all"
                            >
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className={`w-8 h-8 rounded-full bg-gradient-to-tr from-purple-400 to-blue-400 flex items-center justify-center text-white font-bold text-xs ${canNavigate ? 'cursor-pointer' : ''}`}
                                        >
                                            {(post.name || L.forum.anonymous).charAt(0)}
                                        </button>
                                        <button
                                            type="button"
                                            onClick={handleProfileClick}
                                            className={`font-bold text-purple-900 dark:text-purple-300 ${canNavigate ? 'hover:underline cursor-pointer' : ''}`}
                                        >
                                            {post.name || L.forum.anonymous}
                                        </button>
                                    </div>
                                ...
                                <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap pl-10 text-lg leading-relaxed">{post.text}</p>
                            </div>
                        );
                    })
                )}
